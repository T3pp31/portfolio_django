...
def main():
    # 東証上場銘柄一覧を読み込む
    is_file = os.path.isfile("stock_name/data_j.xls")
    if is_file:
        print("東証上場銘柄一覧を読み込みます。")
        df_data_j = pd.read_excel("stock_name/data_j.xls", index_col=None)
    else:
    ...
    df_data_j = df_data_j[
        df_data_j["市場・商品区分"] != "REIT・ベンチャーファンド・カントリーファンド・インフラファンド"
    ]
    ...
    for ticker in tqdm(df_data_j["コード"]):
        ...
        series_ticker_name = pd.concat(
            [series_ticker_name, df_data_j_filter["銘柄名"]]
        )
        series_market_product_category = pd.concat(
            [series_market_product_category, df_data_j_filter["市場・商品区分"]]
        )
        series_type_33 = pd.concat(
            [series_type_33, df_data_j_filter["33業種区分"]]
        )
        series_type_17 = pd.concat(
            [series_type_17, df_data_j_filter["17業種区分"]]
        )

        # 企業情報の指標、財務状況を取得する
        df_company_metrics = pd.concat(
            [df_company_metrics, get_company_metrics(ticker_num, ticker_data)]
        )
        df_company_financial_info = pd.concat(
            [df_company_financial_info, get_company_finacial_info(ticker_data)]
        )
    ...
    ] = series_market_product_category.values
    ...
    df_company_metrics.to_csv(
        "./company_metrics.csv", encoding="cp932", index=False, errors="ignore"
    )
    df_company_financial_info.to_csv(
...
def get_company_metrics(ticker_num, ticker_data):
    ...
    for financial_data_key in financial_data_keys:
        try:
            company_metrics.append(
                ticker_data.financial_data[ticker_num][financial_data_key]
            )
        except Exception as e:
...
def get_company_finacial_info(ticker_data):
    ...
    try:
        ...
        past_stockholdersequity = balance_sheet[
            ["asOfDate", "StockholdersEquity"]
        ]
    ...
    common_asofdate = (
        set(past_totalassets_asofdate)
        .intersection(set(past_netincome_asofdate))
        .intersection(set(past_stockholdersequity_asofdate))
        .intersection(set(past_totalrevenue_asofdate))
    )
    ...
    for asofdate in common_asofdate:
        result_past_totalrevenue = pd.concat(
            [
                result_past_totalrevenue,
                past_totalrevenue[past_totalrevenue["asOfDate"] == asofdate],
            ]
        )
        result_past_netincome = pd.concat(
            [
                result_past_netincome,
                past_netincome[past_netincome["asOfDate"] == asofdate],
            ]
        )
        result_past_stockholdersequity = pd.concat(
            [
                result_past_stockholdersequity,
                past_stockholdersequity[
                    past_stockholdersequity["asOfDate"] == asofdate
                ],
            ]
        )
        result_past_totalassets = pd.concat(
            [
                result_past_totalassets,
                past_totalassets[past_totalassets["asOfDate"] == asofdate],
            ]
        )
    ...
    result_past_stockholdersequity = (
        result_past_stockholdersequity.sort_values("asOfDate")
    )
    result_past_totalassets = result_past_totalassets.sort_values("asOfDate")

    # 過去の自己資本比率を計算する
    ticker_past_capitaladequacyratio = (
        result_past_stockholdersequity["StockholdersEquity"]
        / result_past_totalassets["TotalAssets"]
    )

    # 過去の自己資本利益率を計算する
    ticker_past_roe = (
        result_past_netincome["NetIncome"]
        / result_past_stockholdersequity["StockholdersEquity"]
    )

    # 企業情報の過去の指標を保存する
    # 過去の売上高,純利益,総資産,自己資本比率,自己資本利益率
    df_financial_info = pd.DataFrame(
        {
            "asOfDate": result_past_totalrevenue["asOfDate"],
            "TotalRevenue": result_past_totalrevenue["TotalRevenue"],
            "StockholdersEquity": result_past_stockholdersequity[
                "StockholdersEquity"
            ],
